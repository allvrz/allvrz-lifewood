document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===============================================
    // == ADMIN LOGIN, PROTECTION, & LOGOUT (FIREBASE) ==
    // ===============================================

    // --- Part 1: Login Form Logic (admin.html) ---
    const loginForm = document.getElementById('loginForm');
    if (loginForm) {
        loginForm.addEventListener('submit', (e) => {
            e.preventDefault();
            // In the admin.html, ensure the username input has type="email"
    ...
    // --- Part 2: Dashboard Protection & Logout (admin-dashboard.html) ---
    if (window.location.pathname.endsWith('admin-dashboard.html')) {
        // Use Firebase's real-time auth state checker
        auth.onAuthStateChanged(user => {
            ...
            // If a user is logged in, the page will load normally.
        });
    ...
    // ===============================================
    // == DASHBOARD TABS LOGIC (Unchanged)         ==
    // ===============================================
    ...
    // ===============================================
    // == FORM SUBMISSION LOGIC (FIREBASE)          ==
    // ===============================================
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function (e) {
            ...
            // Add the concern to the 'concerns' collection in Firestore
            db.collection("concerns").add({
                name: formData.get('name'),
                email: formData.get('email'),
                interests: formData.getAll('interest').join(', ') || 'General Inquiry',
                message: formData.get('message'),
                submittedOn: firebase.firestore.FieldValue.serverTimestamp(), // Use server time
                status: 'unresolved' // Set initial status
            })
            .then(() => {
    ...
    if (applicationForm) {
        ...
        applicationForm.addEventListener('submit', (e) => {
            ...
            .then(() => {
                ...
                if(closeBtn) closeBtn.click(); // Close the modal on success
            })
            .catch((error) => {
    ...
    // ====================================================
    // == ADMIN DASHBOARD DATA RENDERING & ACTIONS (FIREBASE) ==
    // ====================================================
    if (window.location.pathname.endsWith('admin-dashboard.html')) {
        ...
        async function acceptApplication(docId) {
            await db.collection('applications').doc(docId).update({ status: 'accepted' });
        ...
        pendingTableBody.addEventListener('click', (e) => {
            if (e.target && e.target.classList.contains('accept')) {
                if (confirm('Are you sure you want to accept this application?')) {
                    ...
                    acceptApplication(e.target.dataset.docId);
                }
            }
        });
        ...
        async function renderConcernsTables() {
            ...
            if (unresolvedSnapshot.empty) {
                ...
                unresolvedSnapshot.forEach(doc => {
                    const con = doc.data();
                    const row = document.createElement('tr');
                    row.innerHTML = `<td>${con.name}</td><td>${con.email}</td><td>${con.interests}</td><td><span title="${con.message}">${con.message.substring(0, 50)}...</span></td><td>${con.submittedOn.toDate().toLocaleString()}</td><td><button class="action-btn resolve" data-doc-id="${doc.id}">Resolve</button></td>`;
            ...
            if (resolvedSnapshot.empty) {
                ...
                resolvedSnapshot.forEach(doc => {
                    const con = doc.data();
                    const row = document.createElement('tr');
                    row.innerHTML = `<td>${con.name}</td><td>${con.email}</td><td>${con.interests}</td><td><span title="${con.message}">${con.message.substring(0, 50)}...</span></td><td>${con.submittedOn.toDate().toLocaleString()}</td>`;
                    resolvedTableBody.appendChild(row);
                });
            }
        }

        async function resolveConcern(docId) {
            await db.collection('concerns').doc(docId).update({ status: 'resolved' });
            renderConcernsTables();
        }
        
        unresolvedTableBody.addEventListener('click', (e) => {
            if (e.target && e.target.classList.contains('resolve')) {
                if (confirm('Are you sure you want to mark this concern as resolved?')) {
                    e.target.disabled = true; // Prevent double clicks
                    resolveConcern(e.target.dataset.docId);
                }
            }
        });
    ...
    if(playBtn && videoModal) {
        ...
        const openVideoModal = () => { videoModal.classList.add('is-active'); modalIframe.src = youtubeURL; };
        const closeVideoModal = () => { videoModal.classList.remove('is-active'); modalIframe.src = ""; };
    ...
    if (projectModal) {
        ...
        openModalButtons.forEach(button => {
            button.addEventListener('click', (e) => { e.preventDefault(); openProjectModal(button.dataset.project); });
        });
    ...
    // --- Keydown event listener for all modals ---
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            document.querySelectorAll('.project-modal.is-active').forEach(modal => {
